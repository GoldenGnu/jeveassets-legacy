...
public class AssetsGetter extends AbstractApiGetter<AssetListResponse> {
	...
	protected AssetListResponse getResponse(final boolean bCorp) throws ApiException {
		if (bCorp) {
			return new com.beimin.eveapi.parser.corporation.AssetListParser().getResponse(Owner.getApiAuthorization(getOwner()));
		} else {
			return new com.beimin.eveapi.parser.pilot.AssetListParser().getResponse(Owner.getApiAuthorization(getOwner()));
		}
	}
