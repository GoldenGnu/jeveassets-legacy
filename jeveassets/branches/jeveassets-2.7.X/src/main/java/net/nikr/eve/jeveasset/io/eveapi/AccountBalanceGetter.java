...
public class AccountBalanceGetter extends AbstractApiGetter<AccountBalanceResponse> {
	...
	protected AccountBalanceResponse getResponse(final boolean bCorp) throws ApiException {
		if (bCorp) {
			return new com.beimin.eveapi.parser.corporation.AccountBalanceParser().getResponse(Owner.getApiAuthorization(getOwner()));
		} else {
			return new com.beimin.eveapi.parser.pilot.AccountBalanceParser().getResponse(Owner.getApiAuthorization(getOwner()));
		}
	}
