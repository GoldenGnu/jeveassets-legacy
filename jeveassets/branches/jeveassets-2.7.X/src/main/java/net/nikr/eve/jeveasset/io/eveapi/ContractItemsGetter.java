...
public class ContractItemsGetter extends AbstractApiGetter<ContractItemsResponse> {
	...
	private final Map<Long, List<ContractItem>> savedItems = new HashMap<Long, List<ContractItem>>();
	...
	protected ContractItemsResponse getResponse(boolean bCorp) throws ApiException {
		if (bCorp) {
			return new com.beimin.eveapi.parser.corporation.ContractItemsParser().getResponse(Owner.getApiAuthorization(getOwner()), currentContract.getContractID());
		} else {
			return new com.beimin.eveapi.parser.pilot.ContractItemsParser().getResponse(Owner.getApiAuthorization(getOwner()), currentContract.getContractID());
		}
	}
